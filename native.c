...
native_priority_to_node(pe_resource_t * rsc, pe_node_t * node)
{
    ...
    if (match) {
        node_score = match->weight;
    }
crm_info("#### YAMAUCHI node : %s score : %d", node->details->uname, node_score);
    if (rsc->role == RSC_ROLE_MASTER && node_score > -INFINITY) {
        // Promoted instance takes base priority + 1
        priority = rsc->priority + 1;
crm_info("#### YAMAUCHI native_priority_to_node() Master priority : %d", priority);
crm_info("#### YAMAUHCI rsc: %s role : %d", rsc->id, rsc->role);

    } else {
        priority = rsc->priority;
crm_info("#### YAMAUCHI native_priority_to_node() No Master priority : %d", priority);
crm_info("#### YAMAUHCI rsc: %s role : %d", rsc->id, rsc->role);
    }
